...
function SalesMonthTrendChart({
  ...
  const chart = useMemo(() => {
    ...
    for (let i = 1; i < points.length; i++) {
      if (points[i]!.revenue > points[peakIdx]!.revenue) peakIdx = i;
    }
    const monthTitle = monthLabelFromIso(points[0]!.date);
    ...
    const padX = 4;
    const padY = 12;
    const innerW = lineW - padX * 2;
    const innerH = lineH - padY * 2;
    const bottomY = padY + innerH;
    const coords = points.map((p, i) => {
      const x = padX + (points.length <= 1 ? innerW / 2 : (i / (points.length - 1)) * innerW);
      const y = padY + innerH - (p.revenue / maxRevenue) * innerH;
      return { x, y, p, i };
    });
  ...
  if (loading) {
    return (
      <div className="dash-sales-chart dash-sales-chart--loading" aria-busy="true">
        <div className="dash-sales-chart__skeleton-kpis" />
        <div className="dash-sales-chart__skeleton-chart" />
      </div>
    );
  }
  if (!chart) {
  ...
  return (
    <div className="dash-sales-chart">
      ...
      <div className="dash-sales-chart__kpis">
        <div className="dash-sales-chart__kpi dash-sales-chart__kpi--primary">
          <span className="dash-sales-chart__kpi-label">Acumulado no mês</span>
          <strong className="dash-sales-chart__kpi-value">{formatBRL(chart.totalRevenue)}</strong>
          <span className="dash-sales-chart__kpi-hint">{chart.totalCount} venda(s)</span>
        </div>
        <div className="dash-sales-chart__kpi">
          <span className="dash-sales-chart__kpi-label">Melhor dia</span>
          <strong className="dash-sales-chart__kpi-value">{formatBRL(points[chart.peakIdx]!.revenue)}</strong>
          <span className="dash-sales-chart__kpi-hint">{formatDate(points[chart.peakIdx]!.date)}</span>
        </div>
        <div className="dash-sales-chart__kpi dash-sales-chart__kpi--focus">
          <span className="dash-sales-chart__kpi-label">
            {hoverIdx != null ? 'Dia selecionado' : 'Passe o mouse no gráfico'}
          </span>
          <strong className="dash-sales-chart__kpi-value">{formatBRL(active.revenue)}</strong>
          <span className="dash-sales-chart__kpi-hint">
            {formatDate(active.date)} · {active.count} venda(s)
          </span>
        </div>
      </div>

      <div
        className="dash-sales-chart__figure"
        role="img"
        aria-label={`Gráfico de faturamento diário em ${chart.monthTitle}`}
      >
        <svg
          className="dash-sales-chart__area-svg"
          viewBox={`0 0 ${chart.lineW} ${chart.lineH}`}
          preserveAspectRatio="none"
          aria-hidden
        >
          <defs>
            <linearGradient id="dashSalesAreaFill" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#818cf8" stopOpacity="0.45" />
              <stop offset="100%" stopColor="#6366f1" stopOpacity="0.04" />
            </linearGradient>
            <linearGradient id="dashSalesLineStroke" x1="0" y1="0" x2="1" y2="0">
          ...
          {chart.gridYs.map((y) => (
            <line
              ...
              y2={y}
              className="dash-sales-chart__grid-line"
            />
          ))}
          {chart.areaPath ? <path d={chart.areaPath} fill="url(#dashSalesAreaFill)" /> : null}
          <polyline
            fill="none"
            stroke="url(#dashSalesLineStroke)"
            strokeWidth="2.75"
            strokeLinejoin="round"
            strokeLinecap="round"
            points={chart.linePoints}
            className="dash-sales-chart__trend-line"
          />
          {chart.coords.map((c) => (
            <circle
              key={c.p.date}
              ...
              className={
                c.i === activeIdx
                  ? 'dash-sales-chart__dot dash-sales-chart__dot--active'
                  : 'dash-sales-chart__dot'
              }
            />
          ))}
        </svg>

        <div className="dash-sales-chart__bars">
          {points.map((p, i) => {
            const day = p.date.slice(8, 10);
            const pct = Math.max(4, (p.revenue / chart.maxRevenue) * 100);
            const isToday = p.date === chart.todayIso;
            const isPeak = i === chart.peakIdx;
            const isActive = i === activeIdx;
            return (
              <div
                key={p.date}
                className={[
                  'dash-sales-chart__bar-col',
                  isToday ? 'is-today' : '',
                  isWeekendIso(p.date) ? 'is-weekend' : '',
                  isPeak ? 'is-peak' : '',
                  isActive ? 'is-active' : '',
                ]
                  .filter(Boolean)
                ...
                tabIndex={0}
                role="presentation"
                aria-label={`${formatDate(p.date)}: ${formatBRL(p.revenue)}, ${p.count} vendas`}
              >
                <div className="dash-sales-chart__bar-track">
                  <div
                    className="dash-sales-chart__bar-fill"
                    style={{ height: `${pct}%`, animationDelay: `${i * 18}ms` }}
                  />
                </div>
                <span className="dash-sales-chart__bar-day">{day}</span>
              </div>
            );
          })}
        </div>
